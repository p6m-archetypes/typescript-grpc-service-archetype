...
import type { {{ ProjectName }}Implementation } from '../generated/{{ project_name }}';
...
export function create{{ ProjectName }}Impl(
  db: FastifyInstance['db'],
): {{ ProjectName }}Implementation {
  const mustFind = async (id: string) => {
    const [row] = await db.select().from(items).where(eq(items.id, id));
    if (!row) {
      throw new ServerError(Status.NOT_FOUND, `{{ EntityName }} ${id} not found`);
    }
    return row;
  };
